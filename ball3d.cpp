...
public:
    ...
    CalcMesh(const std::vector<double>& nodesCoords, const std::vector<std::size_t>& tetrsPoints) {
        ...
        for(unsigned int i = 0; i < nodesCoords.size() / 3; i++) {
            ...
            double vX = -w*pointY;
            double vY = w*pointX;
            double vZ = 0.5*pointZ;
            // Модельная скалярная величина распределена как-то вот так
            double smth = cos(pow(pointX, 3) + pow(pointY, 2)) *(pow(pointZ, 2)+pow(pointX, 2) + pow(pointY, 2));
    ...
    void snapshot(unsigned int snap_number) {
        ...
        string fileName = "ball3d-step-" + std::to_string(snap_number) + ".vtu";
...
int main()
{
    ...
    gmsh::finalize();
       ...
       for(unsigned int step = 1; step < 50; step++) {
